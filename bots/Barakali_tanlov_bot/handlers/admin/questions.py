...
from bots.Barakali_tanlov_bot.states import AdminQuestionStates, AdminQuestionImportStates
...
async def export_questions(cb: CallbackQuery, session: AsyncSession) -> None:
    if not await _is_admin(session, cb.from_user.id):
        await cb.answer()
        return
    from bots.Barakali_tanlov_bot.utils.excel import export_questions_to_excel
    questions = await AdminService(session).list_questions()
...
async def download_template(cb: CallbackQuery, session: AsyncSession) -> None:
    if not await _is_admin(session, cb.from_user.id):
        await cb.answer()
        return
    from bots.Barakali_tanlov_bot.utils.excel import generate_questions_template
...
async def import_questions_excel(
    ...
    if not await _is_admin(session, message.from_user.id):
        return
    from bots.Barakali_tanlov_bot.utils.excel import import_questions_from_excel
    import tempfile, os
