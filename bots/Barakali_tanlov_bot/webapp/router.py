...
from bots.Barakali_tanlov_bot.exceptions import (
    AlreadySolvedError,
    QuizFinishedError,
    QuizNotActiveError,
    QuizWaitingError,
)
from bots.Barakali_tanlov_bot.services import QuizService
from bots.Barakali_tanlov_bot.webapp.auth import (
    get_token_from_header,
    verify_init_data_and_issue_token,
    verify_token,
)
from bots.Barakali_tanlov_bot.webapp.schemas import (
    AnswerRequest,
    AnswerResponse,
    AuthRequest,
    AuthResponse,
...
router = APIRouter(prefix="/barakali_tanlov_bot/webapp", tags=["barakali_tanlov_bot-webapp"])
...
async def auth(body: AuthRequest):
    telegram_id, token = verify_init_data_and_issue_token(
        body.init_data,
        settings.BARAKALI_TANLOV_BOT_BOT_TOKEN,
