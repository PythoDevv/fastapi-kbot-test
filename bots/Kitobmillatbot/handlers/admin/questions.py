...
from bots.Kitobmillatbot.exceptions import QuestionDeletionBlockedError
from bots.Kitobmillatbot.keyboards import inline, reply
from bots.Kitobmillatbot.services import AdminService
from bots.Kitobmillatbot.states import AdminQuestionStates, AdminQuestionImportStates
...
async def download_template(cb: CallbackQuery, session: AsyncSession) -> None:
    if not await _is_admin(session, cb.from_user.id):
        await cb.answer()
        return
    from bots.Kitobmillatbot.utils.excel import generate_questions_template
...
async def import_questions_excel(
    ...
    if not await _is_admin(session, message.from_user.id):
        return
    from bots.Kitobmillatbot.utils.excel import import_questions_from_excel
    import tempfile, os
