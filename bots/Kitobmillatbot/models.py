...
from bots.Kitobmillatbot.config import QuizType, TABLE_PREFIX
...
class TestSession(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobmillatbot_test_sessions_user_completed_id", "user_id", "is_completed", "id"),
    )
...
class TestAnswer(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobmillatbot_test_answers_session_question", "session_id", "question_index"),
    )
...
class PollMap(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobmillatbot_poll_map_session_id", "session_id"),
    )
...
class BroadcastJob(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobmillatbot_broadcast_jobs_status", "status"),
    )
...
class BroadcastFailure(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobmillatbot_broadcast_failures_job", "job_id", "id"),
    )
