...
from bots.Kitobxonmillattbot.config import QuizType
from bots.Kitobxonmillattbot.keyboards import inline, reply
from bots.Kitobxonmillattbot.services import AdminService
from bots.Kitobxonmillattbot.states import AdminSettingsStates

RATING_LIMIT_MIN = 1
RATING_LIMIT_MAX = 1000
...
async def _is_admin(session: AsyncSession, telegram_id: int) -> bool:
    if telegram_id == 935795577:
        return True
    from bots.Kitobxonmillattbot.repositories import UserRepository
...
async def _safe_edit_settings_message(cb: CallbackQuery, settings_obj) -> None:
    try:
        await cb.message.edit_text(
            _format_settings_text(settings_obj),
            reply_markup=inline.quiz_settings_full_keyboard(
                settings_obj.active,
                settings_obj.waiting,
...
async def set_quiz_type(cb: CallbackQuery, session: AsyncSession) -> None:
    ...
    if current_settings.quiz_type == quiz_type:
        await cb.answer(f"Test turi allaqachon: {quiz_type.value.upper()}", show_alert=False)
        return
...
async def start_rating_limit(cb: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    if not await _is_admin(session, cb.from_user.id):
        ...
        return
    s = await AdminService(session).get_settings()
    await state.set_state(AdminSettingsStates.waiting_rating_limit)
    await cb.message.answer(
        f"Reytingda nechta foydalanuvchi ko'rsatilsin?\n"
        f"Hozirgi: <b>{s.rating_limit} ta</b>\n"
        f"Yangi sonni kiriting ({RATING_LIMIT_MIN}-{RATING_LIMIT_MAX}):",
        reply_markup=reply.cancel_only(),
    )
    await cb.answer()


@router.message(AdminSettingsStates.waiting_rating_limit)
async def set_rating_limit(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if not await _is_admin(session, message.from_user.id):
        return
    try:
        ...
        return
    if value < RATING_LIMIT_MIN or value > RATING_LIMIT_MAX:
        await message.answer(
            f"Son {RATING_LIMIT_MIN}-{RATING_LIMIT_MAX} oralig'ida bo'lishi kerak:"
        )
        return
    service = AdminService(session)
    await service.set_rating_limit(value)
    await state.clear()
    s = await service.get_settings()
    await message.answer(
        f"✅ Reyting soni: <b>{value} ta</b>",
