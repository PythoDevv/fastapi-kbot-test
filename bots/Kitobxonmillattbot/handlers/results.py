...
from bots.Kitobxonmillattbot.keyboards import inline
from bots.Kitobxonmillattbot.services import ResultsService
from bots.Kitobxonmillattbot.utils.certificate import build_certificate_input_file, generate_certificate
...
async def show_results(message: Message, session: AsyncSession) -> None:
    ...
    limit = await service.get_rating_limit()
    ...
    top_referrals = await service.top_by_referrals(message.from_user.id, limit=limit)
    top_test_takers = await service.top_test_takers(message.from_user.id, limit=limit)
    ...
    if top_referrals:
        lines.append(f"<b>🏆 Top {limit} referal bo'yicha</b>")
    ...
    if top_test_takers:
        lines.append(f"<b>📝 Top {limit} test yechganlar</b>")
...
async def show_referral_results(cb: CallbackQuery, session: AsyncSession) -> None:
    service = ResultsService(session)
    limit = await service.get_rating_limit()
    user_ref_top = await service.top_by_referrals(cb.from_user.id, limit=limit)
    ...
    if user_ref_top:
        lines.append(f"<b>🏆 Referral bo'yicha top {limit}:</b>")
        for entry in user_ref_top:
    ...
    parts = _split_text("\n".join(lines))
    await cb.message.edit_text(parts[0], reply_markup=inline.results_back_keyboard())
    for part in parts[1:]:
...
async def back_to_results(cb: CallbackQuery, session: AsyncSession) -> None:
    ...
    top_referrals = await service.top_by_referrals(cb.from_user.id, limit=limit)
    top_test_takers = await service.top_test_takers(cb.from_user.id, limit=limit)
