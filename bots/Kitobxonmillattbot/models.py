...
from bots.Kitobxonmillattbot.config import QuizType, TABLE_PREFIX
...
class User(Base, TimestampMixin):
    ...
    test_sessions: Mapped[list["TestSession"]] = relationship(
        "bots.Kitobxonmillattbot.models.TestSession",
...
class UserZayafkaChannel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "zayafka_channel_id", name="uq_kitobxonmillattbot_user_zayafka"),
    )
...
class TestSession(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobxonmillattbot_test_sessions_user_completed_id", "user_id", "is_completed", "id"),
    )
    ...
    user: Mapped["User"] = relationship(
        "bots.Kitobxonmillattbot.models.User",
        back_populates="test_sessions",
    )
    answers: Mapped[list["TestAnswer"]] = relationship(
        "bots.Kitobxonmillattbot.models.TestAnswer",
...
class TestAnswer(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobxonmillattbot_test_answers_session_question", "session_id", "question_index"),
    )
...
class PollMap(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_kitobxonmillattbot_poll_map_session_id", "session_id"),
    )
