...
from bots.Kitobxonmillattbot.services import QuizService
from bots.Kitobxonmillattbot.webapp.auth import (
    get_token_from_header,
    verify_init_data_and_issue_token,
    verify_token,
)
from bots.Kitobxonmillattbot.webapp.schemas import (
    AnswerRequest,
    AnswerResponse,
    AuthRequest,
    AuthResponse,
...
router = APIRouter(prefix="/kitobxonmillattbot/webapp", tags=["kitobxonmillattbot-webapp"])
...
async def auth(body: AuthRequest):
    telegram_id, token = verify_init_data_and_issue_token(
        body.init_data,
        settings.KITOBXONMILLATTBOT_BOT_TOKEN,
