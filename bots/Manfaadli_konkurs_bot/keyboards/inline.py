...
from bots.Manfaadli_konkurs_bot.models import Channel, Question, ZayafkaChannel
from bots.Manfaadli_konkurs_bot.services.quiz_service import QuestionPayload
...
def content_list_keyboard(contents: list | None = None) -> InlineKeyboardMarkup:
    ...
    from bots.Manfaadli_konkurs_bot.models import ContentText
    buttons = []
    if contents:
        for ct in contents:
            buttons.append(
...
def results_main_keyboard() -> InlineKeyboardMarkup:
    """Main results keyboard with Test, Referral and Certificate buttons"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
def certificate_main_keyboard(has_passed: bool) -> InlineKeyboardMarkup:
    """Certificate menu keyboard"""
    buttons = []
    if has_passed:
        buttons.append([InlineKeyboardButton(text="📥 Sertifikatni yuklash", callback_data="cert_generate")])
    buttons.append([InlineKeyboardButton(text="🔙 Orqaga", callback_data="res_back")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def certificate_download_keyboard() -> InlineKeyboardMarkup:
    """Certificate download keyboard"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📥 Sertifikatni yuklash", callback_data="cert_generate")],
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="cert_back")],
        ]
    )


def certificate_from_results_keyboard() -> InlineKeyboardMarkup:
    """Certificate keyboard when accessed from results"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📥 Sertifikatni yuklash", callback_data="cert_generate")],
            [InlineKeyboardButton(text="🔙 Natijalar", callback_data="res_back")],
        ]
    )
