...
from bots.Manfaadli_konkurs_bot.config import QuizType, TABLE_PREFIX
...
class User(Base, TimestampMixin):
    ...
    test_sessions: Mapped[list["TestSession"]] = relationship(
        "bots.Manfaadli_konkurs_bot.models.TestSession",
...
class UserZayafkaChannel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "zayafka_channel_id", name="uq_manfaadli_konkurs_bot_user_zayafka"),
    )
...
class QuizSettings(Base, TimestampMixin):
    ...
    rating_limit: Mapped[int] = mapped_column(Integer, default=30, nullable=False)
...
class TestSession(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_manfaadli_konkurs_bot_test_sessions_user_completed_id", "user_id", "is_completed", "id"),
    )
    ...
    user: Mapped["User"] = relationship(
        "bots.Manfaadli_konkurs_bot.models.User",
        back_populates="test_sessions",
    )
    answers: Mapped[list["TestAnswer"]] = relationship(
        "bots.Manfaadli_konkurs_bot.models.TestAnswer",
...
class TestAnswer(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_manfaadli_konkurs_bot_test_answers_session_question", "session_id", "question_index"),
    )
...
class PollMap(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_manfaadli_konkurs_bot_poll_map_session_id", "session_id"),
    )
