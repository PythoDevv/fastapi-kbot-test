...
from bots.Millatchiroqlaribot.services import QuizService
from bots.Millatchiroqlaribot.webapp.auth import (
    get_token_from_header,
    verify_init_data_and_issue_token,
    verify_token,
)
from bots.Millatchiroqlaribot.webapp.schemas import (
    AnswerRequest,
    AnswerResponse,
    AuthRequest,
    AuthResponse,
...
router = APIRouter(prefix="/millatchiroqlaribot/webapp", tags=["millatchiroqlaribot-webapp"])
...
async def auth(body: AuthRequest):
    telegram_id, token = verify_init_data_and_issue_token(
        body.init_data,
        settings.MILLATCHIROQLARIBOT_BOT_TOKEN,
